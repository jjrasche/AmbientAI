...
fun DatabaseScreen(
    ...
    val logs by logRepository.getAllLogs().collectAsStateWithLifecycle(initialValue = emptyList())
        ...
        TabRow(selectedTabIndex = selectedTab) {
            ...
            Tab(selected = selectedTab == 4, onClick = { selectedTab = 4 }) {
                Box(
                    modifier = Modifier.fillMaxWidth().combinedClickable(
                        onClick = { },
                        onLongClick = { logRepository.deleteAll() }
                    )
                ) {
                    Text(text = "Logs (${logs.size})", modifier = Modifier.padding(16.dp))
                }
            }
        }
                ...
                Column(modifier = Modifier.padding(12.dp)) {
                    Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                        ...
                        Text(
                            ...
                            fontWeight = FontWeight.Bold,
                            color = when (log.type) {
                                "medication" -> MaterialTheme.colorScheme.error
                                "food" -> MaterialTheme.colorScheme.tertiary
                                "supplement" -> MaterialTheme.colorScheme.primary
                                "activity" -> MaterialTheme.colorScheme.secondary
                                "symptom" -> MaterialTheme.colorScheme.error
                                else -> MaterialTheme.colorScheme.onSurface
                            }
                        )
                    }
