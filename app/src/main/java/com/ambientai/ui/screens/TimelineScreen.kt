...
@Composable
fun TimelineScreen(currentTranscript: String, transcriptRepository: TranscriptRepository, actionExecutionRepository: ActionExecutionRepository,
    onNavigateToDb: () -> Unit, onToggleExcludeFromContext: (Transcript) -> Unit) {
    ...
    Column(modifier = Modifier.fillMaxSize().systemBarsPadding().padding(16.dp)) {
        Row(modifier = Modifier.fillMaxWidth().padding(bottom = 16.dp), horizontalArrangement = Arrangement.SpaceBetween) {
            Text(text = "Ambient AI", style = MaterialTheme.typography.headlineMedium)
            Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                Button(onClick = onNavigateToDb) { Text("Database") }
            }
        }
        Card(modifier = Modifier.fillMaxWidth().padding(bottom = 16.dp),
            colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.primaryContainer)) {
            Column(modifier = Modifier.padding(16.dp)) {
                Text(text = "Live Transcript", style = MaterialTheme.typography.titleSmall, color = MaterialTheme.colorScheme.onPrimaryContainer)
                Spacer(modifier = Modifier.height(8.dp))
                Text(text = currentTranscript.ifEmpty { "Waiting for wake word..." },
                    style = MaterialTheme.typography.bodyMedium, color = MaterialTheme.colorScheme.onPrimaryContainer)
            }
        }
        Text(text = "Timeline (${timelineItems.size}) - Long press transcript to toggle context",
            style = MaterialTheme.typography.titleMedium, modifier = Modifier.padding(bottom = 8.dp))
        LazyColumn(modifier = Modifier.fillMaxSize(), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            items(timelineItems, key = { when (it) {
                is TimelineItem.TranscriptItem -> "t-${it.transcript.id}"
                is TimelineItem.LlmItem -> "l-${it.interaction.id}"
            }}) { item ->
                when (item) {
                    is TimelineItem.TranscriptItem -> TranscriptCard(transcript = item.transcript, onToggleExcludeFromContext = onToggleExcludeFromContext)
