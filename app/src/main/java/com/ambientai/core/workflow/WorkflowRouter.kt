...
class WorkflowRouter @Inject constructor(
    ...
    private fun parseTriggers(workflowJson: String) = runCatching { JSONObject(workflowJson).getJSONArray("triggers").let { triggersArray -> List(triggersArray.length()) { i -> triggersArray.getString(i) } } }.getOrElse { emptyList() }
