...
class GroqLlmService @Inject constructor() {
    ...
    private fun makeApiCall(request: LlmRequest) = (URL(API_URL).openConnection() as HttpURLConnection).run { try { apply { requestMethod = "POST"; setRequestProperty("Content-Type", "application/json"); setRequestProperty("Authorization", "Bearer ${BuildConfig.GROQ_API_KEY}"); connectTimeout = TIMEOUT_MS; readTimeout = TIMEOUT_MS; doOutput = true }; OutputStreamWriter(outputStream).use { it.write(buildRequestJson(request)); it.flush() }; if (responseCode != HttpURLConnection.HTTP_OK) throw Exception("API error $responseCode: ${errorStream?.bufferedReader()?.use { it.readText() } ?: "No error body"}"); parseResponse(BufferedReader(InputStreamReader(inputStream)).use { it.readText() }) } finally { disconnect() } }
