...
class TextToSpeechService(private val context: Context, private val onError: (errorCode: Int) -> Unit = {}) {
    ...
    companion object { private const val UTTERANCE_ID = "ambient_ai_utterance" }
    ...
    suspend fun speak(text: String): Boolean { if (!isInitialized || tts == null) runBlocking { initialize() }; if (text.isBlank()) return false; return suspendCancellableCoroutine { continuation -> tts?.setOnUtteranceProgressListener(object : UtteranceProgressListener() { override fun onStart(utteranceId: String?) {}; override fun onDone(utteranceId: String?) { continuation.resume(true) }; override fun onError(utteranceId: String?) { onError(-1); continuation.resume(false) } }); tts?.speak(applyPronunciationFixes(text), TextToSpeech.QUEUE_FLUSH, null, UTTERANCE_ID)?.let { if (it == TextToSpeech.ERROR) continuation.resume(false) } } }
    fun stop() { tts?.stop() }
