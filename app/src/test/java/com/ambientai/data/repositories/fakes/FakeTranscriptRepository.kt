...
class FakeTranscriptRepository : ITranscriptRepository {
    ...
    override fun getRecentContext(chunks: Int): String = transcripts.values.filter { !it.excludeFromContext }.sortedByDescending { it.timestamp }.take(chunks).reversed().joinToString("\n") { "[${it.timestamp}] ${it.text}" }
    fun clear() = transcripts.clear().also { nextId = 1L }
    fun addTranscript(transcript: Transcript) = save(transcript)
}
